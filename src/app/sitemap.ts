...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const [properties, projects, posts, services] = await Promise.all([
    ...
    const dynamicRoutes: MetadataRoute.Sitemap = [
      ...
      ...services.map(s => ({ url: `${baseUrl}/services/${s.slug}`, lastModified: s.updatedAt, changeFrequency: "monthly" as const, priority: 0.7 })),
    ];
